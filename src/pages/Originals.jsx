...
import OriginalList from "../constant/OriginalList";
...
const Originals = () => {
  ...
  const getDisneyPlusOriginals = async () => {
    try {
      const response = await GlobalApi.getDisneyPlusOriginals();
      setOriginalsList(response.data.results);
    } catch (error) {
      console.error("Error fetching Disney Plus Originals:", error);
    }
  };
...
{/* ORIGINALS LIST SECTION */}
<section>
  <div>
    {GenreList.genres.map((item, index) =>
      index < 100 ? (
        <div key={item.id} className="p-8 mr-8 md:px-16">
          <h2 className="text-[1.2rem] font-semibold">{item.Name}</h2>
          <OriginalList genreId={item.id} /> {/* Ensure OriginalList is a valid component */}
        </div>
      ) : null
    )}
  </div>
